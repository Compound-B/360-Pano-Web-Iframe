...
var playlist = [];
var currentVideo = 0;
var wp_vsp_alt_page_title;
var first_time_playing = true;
var omniPlayerObj = null,
    series_player;
...
$.fn.initSeriesPlayer = function (opts) {
	...
	function playOOVideo(json, embedcode) { 
		...
		if (!adplaying) { 
			...
			series_player = new TWP.ooyala.OoyalaPlayer({
				playerContainer: '#ooyala-player',
				embedCode: embedcode,
                playerId: 'special-player',
				json: json,
				width: 945,
				height: 530,
				showEndscreen: 0,
			...
			series_player.onPlayerEmbedded(function(p) { 
				oo_playerLoaded(p);
			})
	  ...
	  oo_playerLoaded = function(p) {
	  
	   	  if (p.hasCompanionAd) {
                p.prepareCompanionAd({    
                'style':{ 
		               'width': '300px', 
		               'height': '250px', 
		               'display': 'inline-block', 
		               'position': 'absolute',
		               'margin': '150px 0 200px 15px' 
        			} 
        		});
          }
	      ...
	      p.onCompanionAdsReady(function(){
				p.showCompanionAd();
	      });
	      p.onAdStarted(function(p) { adTracker() } )
	      p.onAdCompleted(function(p) { 
	      		adFinished() 
				p.removeCompanionAd();
	      } )
	      p.onPlayCompleted(function(p) { videoFinished() } )
