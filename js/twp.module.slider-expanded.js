...
(function($) {
    TWP.Module.Slider = function(o) {
        ...
        var content_w = wp_slider.width();
        var curr_val = opts.start_pos ? content_w * Number(opts.start_pos) : content_w/2;
        
        // set handle and content mask
        var setPos = function( val, animate ) {
            var containerWidth = wp_slider.width(); 
            var newPos = val === null ? (opts.start_pos ? containerWidth * Number(opts.start_pos) : containerWidth/2) : val;
        ...
        addContent = function() {
            ...
            for (var i=opts.content.length-1; i>=0; i--) {
                ...
                var w = wp_slider.width();
                var content = opts.content_type === 'img' ? '<img src="'+ el +'" style="max-width: ' + w + 'px" />' : el;
